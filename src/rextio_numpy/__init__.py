...
Implements Rextio plugin API 1.5 (``rextio.plugins.api``): the plugin
...
(``axis=<int literal>``), plus bounded non-chained comparisons and
three-argument ``numpy.where`` — to Rust via the ``ndarray`` crate, with
pinned crate injection and the ``rextio_numpy.types`` annotation vocabulary. The
